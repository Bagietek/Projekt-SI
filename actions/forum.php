<?php
	...
	if(isset($_GET['search'])){
		$find = $_GET['search'];
		$find = "%$find%";
		$query = $db->prepare('SELECT u.photo, u.id, u.nick, u.description, f.title, f.content, f.picture, f.id AS fID FROM forum f INNER JOIN user u ON f.userID=u.id WHERE f.title LIKE :search');
